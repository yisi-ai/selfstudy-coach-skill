...
const site = ${literal(session.origin)};
const id = ${literal(session.id)};
...
async function forward() {
  ...
  const value = latest;
  try {
    const response = await fetch('/responses', { method: 'POST', headers, body: value });
    if (!response.ok) throw Error('RESPONSE_NOT_SAVED');
    acknowledged = value;
...
window.addEventListener('message', (event) => {
  ...
  if (message.type === 'gaga.questionnaire.ready') {
    ...
    if (value !== acknowledged) { latest = value; void forward(); }
  }
});
fetch('/session', { headers }).then(async (response) => {
  if (!response.ok) throw Error('SESSION_UNAVAILABLE');
  setup = await response.json();
  const url = new URL('/app/import', site);
  url.hash = new URLSearchParams({ bridgeToken: token, bridgeOrigin: location.origin, bridgeId: id });
  frame.src = url.href;
}).catch(fail);
...
async function serveBridge(directory, requestedPort = 0) {
  const session = load(filename(directory, "session"));
  const original = readQuestionnaire(session.questionnaire);
    ...
    void (async () => {
      ...
      if (request.method === "GET" && request.url === "/session")
        return json(200, {
          ...
          imported: existsSync(responseFile) && Boolean(object(load(responseFile)).questionnaire)
        });
      if (request.method === "GET" && request.url === "/status")
        return json(200, existsSync(responseFile) ? load(responseFile) : { pageState: "waiting" });
      ...
      const input = object(await body(request, 1024 * 1024));
      if (input.type !== "gaga.questionnaire.state" || input.id !== session.id || input.token !== session.token || typeof input.skillOperationVersion !== "string")
        throw new Error("INVALID_RESPONSE");
      const previous = existsSync(responseFile) ? object(load(responseFile)) : {};
      let result;
      if (input.pageState === "error") {
        result = {
          ...previous,
          pageState: "error",
          error: typeof input.error === "string" ? input.error.slice(0, 200) : "CONNECTION_FAILED"
        };
      } else {
        if (!["pending", "running", "completed"].includes(String(input.pageState)))
          throw new Error("INVALID_PAGE_STATE");
        const questionnaire = readQuestionnaireSession(input.questionnaire);
        if (questionnaire.id !== `q${session.id}` || JSON.stringify(questionnaire.questionnaire) !== JSON.stringify(original) || input.url !== `${session.origin}/app/questionnaires/${questionnaire.id}`)
          throw new Error("WRONG_QUESTIONNAIRE");
        if (input.pageState === "completed" && questionnaire.completedAt === null || input.pageState === "running" && questionnaire.completedAt !== null)
          throw new Error("INVALID_COMPLETION");
        result = { pageState: input.pageState, questionnaire, url: input.url };
      }
      save(responseFile, {
        ...result,
        sessionId: session.id,
      ...
      json(400, { error: error instanceof Error ? error.message : "INVALID_REQUEST" });
    });
  });
...
async function main(args) {
  ...
  if (!file || !["start", "resume", "status", "read", "stop", "serve"].includes(command ?? ""))
    throw new Error(
      "Usage: questionnaire-bridge.mjs start <questionnaire.json> --origin <web-origin> --output <new-directory> [--locale en|zh-CN], or resume|status|read|stop <directory>"
    );
  if (command === "serve") {
    const directory2 = path2.resolve(file);
    await serveBridge(directory2, load(filename(directory2, "session")).port ?? 0);
    save(filename(directory2, "ready"), { listening: true });
    return;
  }
  if (command === "resume") {
    await launch(path2.resolve(file), fileURLToPath(import.meta.url));
    return;
  }
  if (command === "start") {
    const option = (name) => options[options.indexOf(name) + 1];
    const origin = options.includes("--origin") ? option("--origin") : void 0;
    ...
    if (!options.includes("--output") || !output)
      throw new Error("--output must name a new session directory");
    const directory2 = path2.resolve(output);
    const questionnaire = readQuestionnaire(load(path2.resolve(file)));
    mkdirSync(directory2, { recursive: false, mode: 448 });
    save(filename(directory2, "session"), {
      ...
      questionnaire,
      createdAt: Date.now(),
      locale: option("--locale") === "zh-CN" ? "zh-CN" : "en"
    });
    await launch(directory2, fileURLToPath(import.meta.url));
    return;
  }
  const directory = path2.resolve(file);
  const session = load(filename(directory, "session"));
  if (command === "read") {
    if (!existsSync(filename(directory, "responses")))
      throw new Error("QUESTIONNAIRE_NOT_COMPLETED");
    const result2 = object(load(filename(directory, "responses")));
    if (result2.pageState !== "completed") throw new Error("QUESTIONNAIRE_NOT_COMPLETED");
    const questionnaire = readQuestionnaireSession(result2.questionnaire);
    if (questionnaire.id !== `q${session.id}` || questionnaire.completedAt === null)
      throw new Error("INVALID_COMPLETION");
  ...
  if (!response.ok) throw new Error("Local connection request failed: " + response.status);
  const result = object(await response.json());
  if (command === "status" && result.questionnaire) {
    const questionnaire = readQuestionnaireSession(result.questionnaire);
    result.questionnaire = {
      id: questionnaire.id,
      title: questionnaire.questionnaire.title,
      questionCount: questionnaire.answers.length,
      answeredCount: questionnaire.answers.filter((answer) => answer.optionIds.length).length,
