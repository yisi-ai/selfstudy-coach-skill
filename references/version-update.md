...
> 网页对应的 Skill 已更新到 0.4.18，你当前使用 0.4.17。是否由我更新这个 Skill？请提供对应的正式目录或发布入口，我会先备份并保留你的自定义修改。

实际消息使用当前读到的版本，不能照抄示例版本。当前未配置远程更新源；用户已提供正式目录或明确指定来源时直接核验该来源，否则请用户提供，不猜测 GitHub 地址或声称已有公开下载。没有明确同意就不修改已安装 Skill；拒绝或未答复时保持现状，可继续原学习任务和网页操作。同一会话、同一对版本不反复询问；用户主动要求更新或发现网页又要求新的 Skill 版本时再处理。
...
2. **检查可用正式目录。** 核验用户提供的目录或明确指定的发布来源，读取 SKILL.md 和 release.json，选择与当前网页的 Skill 操作版本相同且比已安装版本更新的正式内容，不盲目跟随 main 或任意 latest。如果用户指定 GitHub 仓库，核对对应版本的完整 Skill 文件，不要求存在压缩包或 GitHub Release。如果不存在合适的新版本或无法访问来源，说明结果并保留当前安装；更换来源或降级另需用户明确选择。
