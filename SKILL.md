...
配套网站：<https://www.aiskillonline.com>。用户题库保存在实际答题浏览器的本站存储中。当前 Skill 未配置远程更新源；需要更新时使用用户提供的对应正式目录或明确指定的发布来源，不猜测仓库地址，也不把本地产物当作已公开发布。
...
2. **静默检查版本，仅在需要更新时提示。** 按 [版本与用户确认更新](references/version-update.md) 比较已安装 Skill 版本与网页声明的 Skill 操作版本。版本相同直接继续，不说“版本一致”“无需更新”或汇报检查过程；只有网页要求更新的 Skill 版本时才提示差异并询问是否更新。缺少更新来源时请用户提供对应正式目录或发布入口。用户明确同意后才更新且保留定制，不比较内容哈希。
